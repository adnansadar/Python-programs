...
#     execute X.
#
# The first line contains the number N — the number of files contained in the filesystem. The following N lines contain
# the file names and allowed operations with them, separated by spaces. The next line contains an integer M — the number
# of operations to the files. In the last M lines specify the operations that are requested for files. One file can be
...
for i in range(int(input())):
    ...
    if Op_Permission[operation] in file_permissions[file]:
        print('OK')
    else:
        print('Access denied')